...
{ String msg="";
  ...
  { int val=msg.toInt();  // Convert message to integer (0–8)

    for (uint8_t i=0; i<8; i++) 
      digitalWrite(leds[i], i<val? HIGH:LOW);  // Turn ON first 'val' LEDs
  }

  if (msg.startsWith("LED") && msg.length() == 4) 
  { int ledIndex=msg.charAt(3)-'0';  // Extract LED number (1–8)

    if (ledIndex>=1 && ledIndex<=8) 
    { digitalWrite(leds[ledIndex-1],HIGH);
      delay(2000);
      digitalWrite(leds[ledIndex-1],LOW);
    }
  }
}
...
{ uint8_t error=read_dht(temperature, humidity, 33, DHT11);
    ...
    dtostrf(temperature, 4, 2, tempStr);  // (float, width, precision, buffer)
    dtostrf(humidity, 4, 2, humStr);

    client.publish("aim/Temperature",tempStr);
    client.publish("aim/Humidity",humStr);
	}
}
